            ...
            <div class="portlet light portlet-fit portlet-datatable bordered">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green sbold uppercase">Manage Skill</span>
                    </div>
                    <div class="actions">
                        <div class="btn-group btn-group-devided">
                            <a href="{{ route('admin.hero.skill_create',$id) }}" class="btn btn-circle green">
                    ...
                    <div class="table-container">
                        <table class="table table-striped table-bordered table-hover" id="sample_3">
                            ...
                            @foreach($model as $row)
                                <tr>
                                    <td> {{ $no }} </td>
                                    <td> <img class="img-thumbnail img-responsive" src="{{ url('assets/skills/'.$row->img) }}" style="max-width:64px;max-height: 64px;"/> </td>
                                    <td> {{ $row->name }} </td>
                                    <td> {{ $row->key }} </td>
                                    <td class="center" width="130">
                                        <a href="{{ route('admin.hero.skill_edit',['id'=>$row->id,'hero'=>$id]) }}" class="btn yellow-saffron btn-xs"><i class="fa fa-pencil"></i></a>
                                    </td>
                                </tr>
                                <?php $no++ ?>
                            @endforeach
